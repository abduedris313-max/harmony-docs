  ...
  return (
    <div className="bg-white border-b border-slate-200 px-3 py-1.5 flex overflow-x-auto md:flex-wrap items-center justify-between gap-2 text-slate-600 scrollbar-none whitespace-nowrap">
      ...
      <div className="flex items-center space-x-1.5 shrink-0">
        ...
        {/* Format Document Button with Auto-Save Format Status */}
        <div className="flex items-center space-x-0.5">
          <button
            onClick={onCleanFormat}
            className="p-1.5 hover:bg-blue-50 text-slate-600 hover:text-blue-600 rounded-l transition-colors text-xs flex items-center space-x-1 font-medium"
            title="Format Document Now (Normalize lists, header spacing, and blank lines)"
          >
            <AlignLeft className="w-4 h-4 text-blue-600" />
            <span className="hidden lg:inline text-[11px] font-semibold text-slate-700">Format</span>
          </button>
          {onOpenSettings && (
            <button
              onClick={onOpenSettings}
              className={`px-1.5 py-0.5 rounded-r text-[9px] font-bold transition-all border ${
                formatOnAutoSave
                  ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'
                  : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200'
              }`}
              title={`Auto-Format on Auto-Save: ${formatOnAutoSave ? 'ENABLED (Click to configure)' : 'DISABLED (Click to configure)'}`}
            >
              {formatOnAutoSave ? 'Auto-Fmt ON' : 'Auto-Fmt OFF'}
            </button>
          )}
        </div>
        ...
        {onOpenAiSummary && (
          <button
            ...
            className="px-2.5 py-1 bg-gradient-to-r from-purple-50 to-blue-50 hover:from-purple-100 hover:to-blue-100 border border-purple-200 text-purple-700 hover:text-purple-900 rounded text-xs flex items-center space-x-1 transition-all shadow-2xs font-semibold"
            title="Generate AI Key Takeaways summary at top of document"
          >
            <Sparkles className="w-3.5 h-3.5 text-purple-600 animate-pulse" />
            <span className="text-[11px]">Key Takeaways</span>
          </button>
        )}
